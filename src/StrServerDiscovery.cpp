...
namespace MorphSyncTogether::StrServerDiscovery
{
    namespace
    {
            ...
            std::size_t offset)
        {
            ...
            // We deliberately parse just this tiny envelope instead of linking
            // LevelDB, because the plugin only needs a couple of simple keys.
            ...
            for (const auto& entry : std::filesystem::directory_iterator(kLocalStoragePath, ec)) {
                ...
                while ((pos = data.find(key, pos)) != std::string::npos) {
                    ...
                    result.push_back(StoredValue{
                        ...
                        order++,
                        TryReadChromiumLocalStorageValue(data, pos + key.size()) });
                    pos += key.size();
                }
            }

            std::ranges::sort(result, [](const StoredValue& lhs, const StoredValue& rhs) {
            ...
            std::string_view key)
        {
            ...
            while (std::getline(file, line)) {
                ...
                if (line.starts_with('[') && line.ends_with(']')) {
                    ...
                    inSection = _stricmp(current.c_str(), std::string(section).c_str()) == 0;
                    continue;
                }
    ...
    ClientState ReadClientState(std::uint16_t morphSyncPort)
    {
        ...
        if (const auto address = ReadLatestLocalStorageValue("last_connected_address")) {
            ...
            state.remotePeer = ParseStrAddress(*address, morphSyncPort);
        }
